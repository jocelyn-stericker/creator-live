        ...
        for (int j=0;j<v->rep->m_sidekicks.size();j++) {
            if (m_out==v->rep->m_sidekicks[j]) {
                ...
                for (int k=0;k<ui->listWidget_vstis->count();k++) {
                    if (ui->listWidget_vstis->item(k)->text()==Vst::m_map.keys()[i]) {
                        qDebug() << "Found (II)";
                ...
                for (int k=0;k<ui->comboBox_instance->count();k++) {
                    qDebug() << "COMPARE"<<ui->comboBox_instance->itemText(k)<<Vst::m_map.values()[i].first;
                    if (ui->comboBox_instance->itemText(k)==Vst::m_map.values()[i].first) {
                        qDebug() << "Found (III)";
...
void VstSelectionWidget::vstChangedLogic()
{
    ...
    for (int i=0;i<Vst::m_map.size();i++) {
        if (Vst::m_map.keys()[i]==ui->listWidget_vstis->currentItem()->text()&&Vst::m_map.values()[i].second.valid()) {
...
void VstSelectionWidget::instanceChangedLogic(int z)
{
    if (!z) {
        ...
        if (m_out.valid()) emit instrumentUpdated(m_out=live::ObjectPtr(),m_loopback);
    } else {
        ...
        if (ui->listWidget_vstis->currentItem()) list=Vst::m_map.values(ui->listWidget_vstis->currentItem()->text());
...
void VstSelectionWidget::loopbackChangedLogic(int z)
{
    if (z==-1) return;
    QList<live::ObjectPtr> list=live::object::get(live::AudioOnly|live::OutputOnly|live::NoRefresh);
    if (m_loopback!=list[z]) emit instrumentUpdated(m_out,m_loopback=list[z]);
}
...
void VstSelectionWidget::chanChangedLogic(int z)
{
    ...
    if (m_out!=a->rep->m_sidekicks[z]) emit instrumentUpdated(m_out=a->rep->m_sidekicks[z],m_loopback);
}
...
void VstSelectionWidget::clickedLogic()
{
    if (!ui->comboBox_instance->currentIndex()) {
        ...
        if (ui->listWidget_vstis->currentItem()) Vst::m_map.values(ui->listWidget_vstis->currentItem()->text());
        for (int i=0;i<list.size();i++) {
            if (list[i].first==r2) {
                a=list[i].second;
                break;
