...
int MidiFile::addEvent(int aTrack, int aTime, Array<uchar>& midiData) {
   ...
   return (int)events[aTrack]->getSize() - 1;
}
...
int MidiFile::addEvent(MFEvent& mfevent) {
   if (getTrackState() == TRACK_STATE_JOINED) {
      ...
      return (int)events[0]->getSize()-1;
   } else {
      events[mfevent.track]->append(mfevent);
      return (int)events[mfevent.track]->getSize()-1;
   }
}
...
int MidiFile::addMetaEvent(int aTrack, int aTime, int aType,
   ...
   timemapvalid = 0;
   int i;
   int length = (int)metaData.getSize();
...
void MidiFile::allocateEvents(int track, int aSize) {
   int oldsize = (int)events[track]->getSize();
   if (oldsize < aSize) {
...
int MidiFile::getNumEvents(int aTrack) { 
   return (int)events[aTrack]->getSize();
}
...
int MidiFile::read(const char* aFile) { 
   ...
   if (signeddata <= 0) {
      ...
      signeddata = (short int)(framespersecond * resolution);
...
void MidiFile::splitTracks(void) { 
   ...
   int maxTrack = 0;
   int i;
   int length = (int)events[0]->getSize();
   for (i=0; i<length; i++) {
...
int MidiFile::linearTickInterpolationAtSecond(double seconds) {
   ...
   int i;
   double lasttime = timemap[(int)timemap.getSize()-1].seconds;
   // give an error value of -1 if time is out of range of data.
   if (seconds < 0.0) {
      return -1;
   }
   if (seconds > timemap[(int)timemap.getSize()-1].seconds) {
      return -1;
   }
   ...
   if (seconds < lasttime / 2) {
      ...
      for (i=(int)timemap.getSize()-1; i>0; i--) {
         if (timemap[i].seconds < seconds) {
            startindex = i+1;
            break;
         } else if (timemap[i].seconds == seconds) {
   ...
   return (int)((int)xi-(int)x1) * (((int)y2-(int)y1)/((int)x2-(int)x1)) + (int)y1;
}
